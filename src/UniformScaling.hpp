...
class UniformScaling : public TransformBase<N, UniformScaling<T, N>>
{
  public:
    ...
    constexpr auto operator()(const A &xi) const noexcept
    {
        ...
        static_for<0, N, 1>([&](auto I) { transformed[I()] = m_scaling * get<I()>(xi) + m_trans[I()]; });
        return transformed;
    }
    ...
    SUBCASE("A one-dimensional uniform scaling transformation with no volume change")
    {
        ...
        REQUIRE(
            transform.partial<0>(Polynomials::make_poly(
                std::tuple(Rationals::rational<2>), Polynomials::PowersList<Polynomials::Powers<2>>{}))(
                std::tuple(0.2)) == doctest::Approx(0.8));

        REQUIRE(
            transform.integrate<3>(Polynomials::make_poly(
                std::tuple(1), Polynomials::PowersList<Polynomials::Powers<3>>{})) == doctest::Approx(0.0));
